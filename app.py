...
async def delete_message(message_id: int, username: str):
    ...
    db_conn.commit()
    return {"status": "deleted" if cursor.rowcount else "not_found"}

@app.post("/messages/{message_id}/forward")
async def forward_message(message_id: int, recipient: str, username: str):
    cursor.execute("SELECT type, content FROM messages WHERE id = ? AND (sender = ? OR recipient = ?)", (message_id, username, username))
    row = cursor.fetchone()
    if not row: return {"status": "not_found"}
    cursor.execute("INSERT INTO messages (sender, recipient, type, content) VALUES (?, ?, ?, ?)", (username, recipient, row[0], row[1]))
    db_conn.commit()
    return {"status": "forwarded", "id": cursor.lastrowid}
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', system-ui, sans-serif; }
        ...
        .auth-note { color: var(--text-muted); font-size: 12px; line-height: 1.5; margin-top: 14px; }
        .message-bubble { position: relative; }
        .message-bubble.selected { outline: 2px solid var(--accent); outline-offset: 3px; }
        .msg-actions { display:none; gap:5px; margin-top:5px; }
        .message-bubble:hover .msg-actions, .message-bubble.selected .msg-actions { display:flex; }
        .msg-actions button { border:0; border-radius:6px; padding:3px 6px; cursor:pointer; background:rgba(255,255,255,.12); color:inherit; }
        .loading-overlay { position:fixed; inset:0; display:none; place-items:center; background:rgba(5,8,20,.55); z-index:300; }
...
<body class="theme-dark">
    ...
    <div id="app-container">
        ...
        <div id="login-screen">
            <div id="login-box">
                ...
                <div class="divider">Secure Google account access</div>
                <p class="auth-note">Your Google name, photo, and email identify your profile. Returning users are signed in automatically on this device.</p>
            </div>
        </div>
    ...
    <script>
        ...
        let googleProfile = JSON.parse(localStorage.getItem("metaverse_google_profile") || "null");
        ...
        async function fetchUserData(username) {
            try {
                ...
                userProfilePic = data.profile_pic || (googleProfile && googleProfile.picture) || "";
                userStatus = data.status || userStatus;
        ...
        function handleGoogleLogin(response) {
            try {
                ...
                if (payload.email) {
                    googleProfile = { email: payload.email, name: payload.name || payload.email, picture: payload.picture || "", sub: payload.sub || "" };
                    localStorage.setItem("metaverse_google_profile", JSON.stringify(googleProfile));
                    initializeUserSession(payload.email);
                }
            } catch (err) {
        ...
        async function saveUserProfileToBackend() {
            await fetch("/user/update", {
                ...
                body: JSON.stringify({ username: currentUser, status: userStatus, profile_pic: userProfilePic, theme: currentTheme, display_name: (googleProfile && googleProfile.name) || currentUser, google_sub: (googleProfile && googleProfile.sub) || null })
            });
        }
        ...
        function connectWebSocket() {
            ...
            ws.onmessage = async function(event) {
                ...
                if (data.sender_id && data.sender_id !== currentUser && data.message) showNotification(`New message from ${data.sender_id}`, data.message);
        ...
        function openContactProfile() {
            ...
            const profileRes = await fetch(`/user/${encodeURIComponent(activeContact)}`);
            const profile = await profileRes.json();
            document.getElementById("modalProfileName").innerText = profile.display_name || activeContact;
            document.getElementById("modalProfileStatus").innerText = `${onlineUsers.includes(activeContact) ? "Online" : "Offline"} · ${profile.status || ""}`;
            document.getElementById("modalProfileAvatar").innerHTML = profile.profile_pic ? `<img src="${profile.profile_pic}">` : activeContact.charAt(0).toUpperCase();
                ...
                container.innerHTML += `
                    <div class="message ${isOutgoing ? "outgoing" : "incoming"}">
                        <div class="msg-body">
                            ${senderLabel}
                            ${contentHTML}
                            <div class="msg-actions"><button onclick="event.stopPropagation(); selectMessage(${m.id})">☑ Select</button>${isOutgoing ? `<button onclick="event.stopPropagation(); deleteMessage(${m.id})">🗑 Delete</button>` : ''}<button onclick="event.stopPropagation(); forwardMessage(${m.id})">↗ Forward</button></div>${isOutgoing ? '<span class="msg-ticks">✓✓</span>' : ''}
                        </div>
                    </div>
                `;
            });
        ...
        function selectMessage(id) { selectedMessageId = id; renderMessages(); }
        async function deleteMessage(id) {
            if (!currentUser || !confirm("Delete this message?")) return;
            setLoading(true);
            try { await fetch(`/messages/${id}?username=${encodeURIComponent(currentUser)}`, { method: "DELETE" });
                if (activeContact && chatHistories[activeContact]) chatHistories[activeContact] = chatHistories[activeContact].filter(m => m.id !== id);
                selectedMessageId = null; renderMessages();
            } finally { setLoading(false); }
        }
        async function forwardMessage(id) {
            const recipient = prompt("Forward to Google email or contact:");
            if (!recipient) return;
            setLoading(true);
            try { const r = await fetch(`/messages/${id}/forward?username=${encodeURIComponent(currentUser)}&recipient=${encodeURIComponent(recipient)}`, { method: "POST" });
                if ((await r.json()).status === "forwarded") showNotification("Message forwarded", recipient);
            } finally { setLoading(false); }
        }
